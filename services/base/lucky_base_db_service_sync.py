...
class LuckyDBBaseServiceSync(metaclass=ABCMeta):
    ...
    def init_instance(cls, db_path: str) -> "LuckyDBBaseServiceSync":
        ...
        # cls._instance.test_db_connection()
        return cls._instance
    ...
    def _init_db(cls, db_path: str):
        ...
        try:
            ...
            cls._db_connection.execute("PRAGMA foreign_keys = ON")
            cls._db_connection.execute("PRAGMA query_only = 1")
    ...
    @classmethod
    def execute_query(cls, sql: str, params: Optional[tuple] = None) -> List[Dict[str, Any]]:
        """类方法：执行查询（只读优化+上下文管理器兼容版）"""
        if not cls._db_connection:
            raise DBConnectionNotInitializedError()
        try:
            #  用连接的上下文管理器（而非Cursor），兼容所有版本
            with cls._db_connection:
                cursor = cls._db_connection.execute(sql, params or ())
